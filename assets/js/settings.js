(function () {
	...
	function renderAudioStatus(ep) {
		...
		if (ep.url_error) {
			return '\u26A0\uFE0F <span style="color:#dba617;" title="' + escHtml( ep.url_error ) + '">' + escHtml( config.i18n.urlNotPublic ) + '</span>';
		}
		return '\u2705';
	}
	...
	function renderStatus(status) {
		if ( ! status) {
			return '<span style="color:#646970;">' + escHtml( config.i18n.idle ) + '</span>';
		}
		if (status === 'imported') {
			return '<span style="color:#00a32a;">' + escHtml( config.i18n.completed ) + '</span>';
		}
		if (status === 'completed') {
			return '<span style="color:#00a32a;">' + escHtml( config.i18n.completed ) + '</span>';
		}
		if (status === 'error') {
			return '<span style="color:#d63638;">' + escHtml( config.i18n.failed ) + '</span>';
		}
		if (status === 'queued' || status === 'processing') {
			return '<span style="color:#dba617;">' + escHtml( config.i18n.processing ) + '</span>';
		}
	...
	function bindBatchEvents() {
		...
		if (selectWithoutBtn) {
			selectWithoutBtn.addEventListener( 'click', selectWithoutTranscript );
		}
		if (deselectAllBtn) {
	...
	function updateBatchButton() {
		...
		if (btn) {
			btn.disabled = batchRunning || getSelectedPostIds().length === 0;
		}
	}
	...
	function startBatch() {
		...
		var withTranscripts = episodes.filter(
			function (ep) {
	...
	function processNext(postIds, index) {
		...
		apiFetch( '/transcribe/' + postId, { method: 'POST' } ).then(
			function (res) {
				if ( ! res.ok) {
	...
	function importAndContinue(postId, postIds, index) {
		apiFetch( '/import/' + postId, { method: 'POST' } ).then(
			function (res) {
				if (res.ok) {
	...
	function updateRowStatus(postId, statusHtml) {
		...
		if (row) {
			...
			if (cell) {
				cell.innerHTML = escHtml( statusHtml );
			}
		}
	}
	...
	function updateEpisodeStatus(postId, status) {
		for (var i = 0; i < episodes.length; i++) {
			if (episodes[i].post_id === postId) {
				...
				if (status === 'imported') {
					episodes[i].has_transcript = true;
				}
				break;
			}
		}
