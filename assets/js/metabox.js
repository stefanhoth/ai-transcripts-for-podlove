(function () {
	...
	function onTranscribeAgain() {
		...
		apiFetch( '/config?post_id=' + config.postId ).then(
			function (res) {
	...
	function startTranscription() {
		...
		apiFetch( '/transcribe/' + config.postId, { method: 'POST' } ).then(
			function (res) {
				if ( ! res.ok) {
	...
	function pollStatus() {
		...
		apiFetch( '/status/' + config.postId ).then(
			function (res) {
				if ( ! res.ok) {
	...
	function importTranscript() {
		...
		apiFetch( '/import/' + config.postId, { method: 'POST' } ).then(
			function (res) {
				if ( ! res.ok) {
					setError( res.data.error || config.i18n.error );
					return;
				...
				try {
					sessionStorage.setItem( 'ai-transcripts-scroll', '1' ); } catch (e) {
					}
